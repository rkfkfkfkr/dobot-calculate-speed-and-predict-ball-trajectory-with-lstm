...
def dobot_get_start():
    ...
    dType.SetPTPCommonParams(api, 350, 350, isQueued = 1) # velocityRatio(속도율), accelerationRation(가속율)
...
def predict_control():
    ...
    while(1):
        ...
        if px != None and py != None:
            ...
            t = time.time()    
            print("px: %f, py: %f, time: %.3f" %(px,py,t))
            ...
            if py > ball_y[0] + 10:
                ...
                print('V: %f cm/s' %(velocity))

                predict_t = (124 - ball_y[-1])/velocity

                print('P_time = %f' %(predict_t))
